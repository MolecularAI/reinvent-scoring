...
class Test_pip_hh_clint(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.expected_scores = [1.3, 1.2, 1.3]
...
class Test_pip_hh_clint_transformation(unittest.TestCase):
    def setUp(cls):
        ...
        parameters.specific_parameters[csp_enum.TRANSFORMATION].update({
            TransformationParametersEnum.TRANSFORMATION_TYPE: transf_type.LEFT_STEP,
            TransformationParametersEnum.LOW: 1.33
        })
        ...
        cls.query_smiles = [CELECOXIB, BUTANE, PENTANE]
        cls.expected_raw_scores = [1.3, 1.06, 1.334]
        cls.expected_scores = [1.0, 1.0, 0.0]
        cls.component = PiPLogPredictionComponent(parameters)
